...
export function detectAnomaly(
  ...
  const observed: MetricsObserved = {
  ...
  const repeatSensitivity = classifySensitivity(
    occurrenceCount,
    confidence,
    cfg.minOccurrencesToFlag,
...
function determineAnomalyType(
  ...
  if (durationAnomaly) return 'latency_spike';
  if (cpuAnomaly) return 'cpu_anomaly';
  if (memoryAnomaly) return 'memory_anomaly';
...
function generateReason(
  ...
  switch (anomalyType) {
    case 'latency_spike':
      parts.push(
        `Latency ${observed.durationMs}ms exceeds expected ${Math.round(expected.durationMs)}ms`
      );
    ...
    case 'cpu_anomaly':
      parts.push(
        `CPU delta ${observed.cpuDelta.toFixed(1)}% exceeds expected ${expected.cpu} usage`
      );
    ...
    case 'memory_anomaly':
      parts.push(
        `Memory delta ${observed.memoryDeltaMb.toFixed(1)}MB exceeds expected ${expected.memory} usage`
      );
