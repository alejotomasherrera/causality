 ...
 */
export function generateReplayInstructions(explanation, propagation, inputs) {
    const instructions = [];
    let step = 1;
 ...
 */
function generateActionDescription(prop) {
    const parts = [];
    // Service context
 ...
 */
function generateVerifyDescription(explanation) {
 ...
 */
function hasCleanupNeeded(propagation) {
    // Cleanup needed if any write operations to DB
    return propagation.some((p) => p.service.type === 'db' && p.service.operation !== 'read');
}
/**
 * Format instructions as markdown.
 */
export function formatInstructionsAsMarkdown(instructions) {
