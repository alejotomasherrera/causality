 ...
 */
export function calculateImpactScore(explanation, frequency, config) {
    ...
    const affectedActionsScore = calculateAffectedActionsScore(explanation, weights.affectedActions ?? 20);
    const resourcePressureScore = calculateResourcePressureScore(explanation, weights.resourcePressure ?? 15);
    const scopeScore = calculateScopeScore(explanation, weights.scope ?? 15);
    const total = issueTypeScore +
        frequencyScore +
 ...
 */
export function extractIssueCategory(explanation) {
 ...
 */
export function detectImpactScope(explanation, patterns) {
...
function calculateIssueTypeScore(explanation, maxScore) {
    // Issue type weights
    const typeWeights = {
        explicit_error: 1.0,
...
function calculateFrequencyScore(frequency, maxScore) {
...
function calculateAffectedActionsScore(explanation, maxScore) {
    ...
    const degradedRatio = causalChain.length > 0
...
function calculateResourcePressureScore(explanation, maxScore) {
    ...
    if (resourcePressure.eventLoopBlocked) {
        const lagIntensity = Math.min((resourcePressure.peakEventLoopLagMs ?? 0) / 500, 1);
        score += 0.2 * lagIntensity;
    }
    return maxScore * score;
}
function calculateScopeScore(explanation, maxScore) {
...
function derivePriority(score, thresholds) {
