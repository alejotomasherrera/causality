...
import { classifyIssue, calculateSeverity, generateSummary, } from './classifier.js';
/**
 * Build a reproducibility explanation from trace data.
 * Output is deterministic given the same input.
 */
export function buildExplanation(input, config) {
    ...
    const replayInstructions = generateReplayInstructions(causalChain, conditions, failurePoint);
    return {
 ...
 */
function generateDeterministicId(traceId) {
 ...
 */
function generateReplayInstructions(chain, conditions, failurePoint) {
    const instructions = [];
    let stepNumber = 1;
