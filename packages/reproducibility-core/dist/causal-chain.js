 ...
 */
export function buildCausalChain(trace, metrics, findings) {
    ...
    const steps = flatActions.map((action, index) => {
        ...
        const contributesToDegradation = actionFindings.some((f) => f.type === 'silent_degradation' || f.type === 'slow_action');
        return {
            order: index,
            actionId: action.id,
            name: action.name,
 ...
 */
function findFailurePoint(actions, findings) {
 ...
 */
export function getExecutionPath(chain, actionId) {
 ...
 */
export function getParallelActions(chain) {
    const groups = [];
    const used = new Set();
    for (const step of chain) {
        if (used.has(step.actionId))
            continue;
        if (!step.endTime)
            continue;
        const parallel = [step];
        ...
        for (const other of chain) {
            if (used.has(other.actionId))
                continue;
            if (!other.endTime)
                continue;
            if (other.actionId === step.actionId)
                continue;
            // Check overlap
            const overlaps = step.startTime < other.endTime && step.endTime > other.startTime;
            if (overlaps) {
                parallel.push(other);
 ...
 */
export function calculateMaxConcurrency(chain) {
    if (chain.length === 0)
        return 0;
