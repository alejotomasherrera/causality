 ...
 */
export function classifyIssue(trace, steps, findings) {
    ...
    const hasSilentDegradation = findings.some((f) => f.type === 'silent_degradation');
    if (hasSilentDegradation) {
        return 'silent_degradation';
    }
    // Check for any degradation findings
    const hasDegradation = findings.some((f) => f.type === 'slow_action' ||
        f.type === 'high_cpu' ||
        f.type === 'memory_spike' ||
 ...
 */
export function calculateSeverity(issueType, steps, findings) {
 ...
 */
export function generateSummary(issueType, severity, steps, findings) {
    const parts = [];
 ...
 */
export function getRootCauseAction(steps, findings) {
