...
export function calculateAggregateMetrics(
  ...
  let maxEventLoopLagMs: number | undefined;
  let errorCount = 0;
 ...
 */
export function getImpactfulFindings(
  findings: MetricFinding[],
  limit: number = 5
): MetricFinding[] {
 ...
 */
export function getContributingSteps(steps: CausalStep[]): CausalStep[] {
  return steps.filter(
    (step) =>
      step.status === 'error' ||
      step.isFailurePoint ||
...
export function summarizeEvidence(
  ...
  if (slowFindings.length > 0) {
    ...
    evidence.push(
      `${slowFindings.length} slow action(s), peak duration ${maxDuration}ms`
    );
  }
  ...
  if (memFindings.length > 0) {
    ...
    evidence.push(
      `${memFindings.length} memory spike(s), peak ${maxMem.toFixed(1)}MB`
    );
  }
  ...
  if (silentFindings.length > 0) {
    evidence.push(
      `${silentFindings.length} silent degradation(s) detected`
    );
  }
 ...
 */
export function getPeakResourceStep(
  steps: CausalStep[],
  metric: 'cpu' | 'memory' | 'duration'
): CausalStep | undefined {
  let peak: CausalStep | undefined;
