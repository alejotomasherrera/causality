 ...
 */
function extractRequiredInputs(steps: CausalStep[]): RequiredInput[] {
  const inputs: RequiredInput[] = [];
  const seen = new Set<string>();

  for (const step of steps) {
    if (!step.attributes) continue;
 ...
 */
function extractTimingConstraints(steps: CausalStep[]): TimingConstraint[] {
  ...
  const childrenByParent = new Map<string, CausalStep[]>();
 ...
 */
function extractResourcePressure(
  steps: CausalStep[],
  cpuThreshold: number,
  memThreshold: number,
  lagThreshold: number
  ...
  let peakEventLoopLagMs: number | undefined;
 ...
 */
function extractLoadCharacteristics(steps: CausalStep[]): LoadCharacteristics {
  if (steps.length === 0) {
    return {
      totalActions: 0,
  ...
  const maxEnd = Math.max(
    ...steps.map((s) => s.endTime ?? s.startTime)
  );
 ...
 */
export function inferReproducibilityPattern(
  ...
  if (conditions.concurrencyLevel >= 3) {
    patterns.push(
      `Requires concurrent load (${conditions.concurrencyLevel} parallel actions)`
    );
  }
  ...
  if (conditions.resourcePressure.cpuPressure) {
    patterns.push(
      `Requires CPU stress (peak ${conditions.resourcePressure.peakCpuDelta.toFixed(1)}%)`
    );
  }
  ...
  if (conditions.resourcePressure.memoryPressure) {
    patterns.push(
      `Requires memory pressure (peak ${conditions.resourcePressure.peakMemoryDeltaMb.toFixed(1)}MB)`
    );
  }
  ...
  if (conditions.resourcePressure.eventLoopBlocked) {
    patterns.push(
      `Requires blocking operation (lag ${conditions.resourcePressure.peakEventLoopLagMs?.toFixed(1)}ms)`
    );
  }
  ...
  // Timing constraints
  const parallelConstraints = conditions.timingConstraints.filter(
    (c) => c.type === 'parallel'
  );
  ...
  const delayConstraints = conditions.timingConstraints.filter(
    (c) => c.type === 'delayed'
  );
