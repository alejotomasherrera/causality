 ...
 */
export function detectSlowAction(
 ...
 */
export function detectEventLoopLag(
  metrics: ActionMetrics,
  thresholds: DetectionThresholds
): MetricFinding | null {
  ...
  const severity =
    endLag > maxLag * 3 ? 'high' : endLag > maxLag * 2 ? 'medium' : 'low';
...
export function detectSilentDegradation(
  ...
  if (metrics.durationMs > maxDuration) {
    ...
    if (metrics.durationMs > maxDuration * 2) maxSeverity = 'high';
    else if (metrics.durationMs > maxDuration * 1.5) maxSeverity = 'medium';
  }
...
export function runAllDetectors(
  ...
  if (cpu) findings.push(cpu);

  const memory = detectMemorySpike(metrics, thresholds);
  if (memory) findings.push(memory);
...
function calculateDurationSeverity(
  actual: number,
  threshold: number
): FindingSeverity {
...
function calculateMemorySeverity(
  actual: number,
  threshold: number
): FindingSeverity {
  const ratio = actual / threshold;
