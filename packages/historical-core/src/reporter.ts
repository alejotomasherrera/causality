 ...
 */
export function generateReport(statsList: AggregatedStats[]): HistoricalReport {
 ...
 */
function createReportItem(stats: AggregatedStats): HistoricalReportItem {
 ...
 */
function getMostCommonError(counts: Record<string, number>): string | undefined {
  let max = 0;
 ...
 */
function recommendAttention(stats: AggregatedStats, trend: TrendAnalysis): AttentionLevel {
